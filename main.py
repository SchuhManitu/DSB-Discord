    ...
    def fetch(self, testmode):
        ...
        for i2 in [lesson for day in entries for lesson in day if self.clss in lesson['type']]:
            if i2 not in [lesson for day in cache for lesson in day if self.clss in lesson['type']]:
                type = i2['lesson']
                if type == 'Entfall':
                    msg = 'Am ' + i2['day'] + ' dem ' + i2['date'] + ' entfällt die ' + i2['class'] + '. Stunde (' + i2[
                        'subject'] + ')'
                elif type == 'Vertretung':
                    msg = 'Am ' + i2['day'] + ' dem ' + i2['date'] + ' wird die ' + i2['class'] + '. Stunde (' + i2[
                        'subject'] + ') vertreten '
                else:
                    msg = 'Am ' + i2['day'] + ' dem ' + i2['date'] + ' ist die ' + i2['class'] + '. Stunde (' + i2[
                        'subject'] + ') als \'' + type + '\' gekenzeichnet.'
                ...
                if ' - ' in i2['class']:
                    ...
                    old_messages.append(msg.replace(i2['class'] + '. Stunde',
                                                    i2['class'] + ' - ' + str(int(i2['class']) + 1) + '. Stunde'))

                self.send_dc(username='Vertretungsplan', title="", description=msg.replace('~', '~~'), color="466985",
                             url=self.weebhook_url)
...
if __name__ == '__main__':
    ...
    dsb = DSB(KENNWORT, KENNUNG, KLASSE, WEEBHOOK_URL)
    ...
    while True:
        try:
            ...
            print(sys.exc_info())
            send(
                f'Name: dsb mobile [dc] // Typ: {exc_type} / Info: {exc_obj} / File: {fname} / Line: {exc_tb.tb_lineno} / Version: unaviable')
        time.sleep(500)
